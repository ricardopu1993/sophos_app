defmodule SophosApp.ProjectEuler do
  ...
  defp do_transform([], filtered), do: filtered
  defp do_transform([h|t], filtered) do
    do_transform(t, [evaluate(h) | filtered])
  end
  ...
  defp filter(numbers) do
    # filter_fun = fn n -> rem(h,5) == 0 or rem(h,3) == 0 end
    do_filter(numbers, [])
  end
  ...
  defp apply_filter(h, list) when rem(h,5) == 0, do: [h | list]
  defp apply_filter(h, list) when rem(h,3) == 0, do: [h | list]
  defp apply_filter(_h, list), do: list

  defp apply_filter_2(h, list)
      when rem(h, 5) == 0 or rem(h, 3) == 0,
      do: [h | list]

  defp apply_filter_2(_, list), do: list
end
